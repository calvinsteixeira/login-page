...
const LoginForm = (): React.ReactNode => {
  ...
  const submitLoginWithGoogle = async () => {
    try {
      ...
      if (error.status) {
        // make sure the error was handled on bff
        toast.error(error.message, {
          toastId: "customId",
          position: "top-right",
          autoClose: 3000,
    ...
    <main className="flex w-full min-h-screen flex-col items-center py-12">
      ...
      <div className="w-full space-y-10">
        ...
        <div className="w-full flex mb-12 flex-row justify-around gap-2">
          ...
          >
            {formMode === "login" && <MdOutlineKeyboardArrowRight size={22} className="absolute left-[10%] text-md" />}
            Entrar
          </span>
          <span
            ...
            className={`relative flex-1 flex justify-center items-center font-semibold px-4 py-3 text-primary gap-4`}
          >
            {formMode === "register" && <MdOutlineKeyboardArrowRight size={22} className="absolute left-[1%] text-md" />}
            Cadastrar
          </span>
        </div>
